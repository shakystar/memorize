...
실사용 외에, 공개 500문항 장기기억 벤치마크 [LongMemEval-S](https://github.com/xiaowu0162/longmemeval)로 검색 품질도 측정합니다. 각 질문의 답은 과거 대화 세션 약 50개 중 하나에 묻혀 있습니다. 그 세션들을 memorize에 넣고 맞는 세션을 되찾아오는지 확인합니다.

| 모드 | recall@5 | recall@10 | recall@20 | ndcg@10 | mrr |
| --- | --- | --- | --- | --- | --- |
| 어휘 검색 (BM25) | 0.966 | 0.986 | 0.994 | 0.896 | 0.911 |
| 하이브리드 (BM25 + bge-m3) | 0.978 | 0.994 | 1.000 | 0.925 | 0.932 |

어휘 검색만으로도 질문의 96.6%에서 맞는 세션을 상위 5개 안에 찾아냅니다. 의미 검색은 질문과 세션이 다른 단어를 쓰는 경우(선호나 바꿔 말한 사실)에서 가장 도움이 되고, 맞는 세션을 더 위로 끌어올립니다. 이 수치는 retrieval recall이지 최종 답변 정확도가 아니며, 위에 얹힌 consolidation이 아니라 검색 계층 자체를 측정합니다. `pnpm benchmark:retrieval bm25`로 재현할 수 있습니다.
...
- **임베딩 기반 검색은 위 벤치마크에서는 검증됐지만, 장기 실사용 검증은 아직입니다.**
- **결정 포착이 파일 쓰기와 명령 쪽에 치우쳐 있습니다**(신호의 99.8%). 명시적 결정 키워드 포착은 개선 중입니다.

임베딩 검색과 결정 포착은 진행 중인 다음 마일스톤입니다.
