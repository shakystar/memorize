...
1. **捕获** — 智能体工作时,钩子记录廉价的规则过滤观察(文件修改、
   决策、任务流转)。无 LLM,零延迟。
2. **巩固(consolidation)** — 在会话边界,后台进程把观察和对话本身蒸馏成
   长期记忆(决策、理由、进展)并打上重要度分数。提取通过你已登录的
...
memorize update            # 升级 CLI 并刷新本机所有集成
memorize session activity  # 我的其他会话在做什么?
memorize consolidate       # 立即运行一次记忆巩固边界
memorize search <query>    # 搜索项目记忆
memorize project show      # 输出绑定项目摘要 (JSON)
memorize version           # 实际运行的二进制版本
```

任务与交接(`memorize task …`)是用于智能体间显式协调的可选层 —
环境记忆无需它们即可工作,任务列表为空是正常的。
