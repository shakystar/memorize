...
describe('loadStartContext picker — task deconfliction', () => {
  ...
  it('hides heartbeat-stale sessions from the picker without changing their stored status', async () => {
    ...
    // Mark the session stale (old lastSeenAt) directly in the projection
    // table WITHOUT changing its status — mirrors a real session that
    // simply stopped heartbeating.
    const session = getSession(project.id, staleSessionId);
    if (!session) throw new Error('Expected session in projection store.');
    const longAgo = '2020-01-01T00:00:00.000Z';
    const staleSession = { ...session, lastSeenAt: longAgo, updatedAt: longAgo };
    getDb(project.id)
      .prepare('UPDATE sessions SET data = ? WHERE id = ?')
      .run(JSON.stringify(staleSession), staleSessionId);
    ...
    // But the session record is unchanged — still 'active'.
    const stored = getSession(project.id, staleSessionId);
