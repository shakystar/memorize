...
describe('content-safety hardening', () => {
  ...
  describe('wrapUntrusted — actor attribute injection', () => {
    ...
    it('neutralizes actor containing quotes and tag syntax', () => {
    ...
    it('neutralizes actor containing a raw angle bracket', () => {
      ...
      expect(angleBrackets).toBe(1);
    });

    it('neutralizes actor containing a closing sentinel', () => {
      const result = wrapUntrusted('data', {
        source: 'task',
  ...
  describe('detectInjectionMarkers — encoding bypass', () => {
    ...
    it('detects injection across zero-width-space separators', () => {
      ...
      const detected = markers.some((m) => m.name === 'ignore-previous');
      // ZWSP is now normalized away before matching, so this injection
      // phrase is detected even though \s does not match it.
      expect(detected).toBe(true);
    });
